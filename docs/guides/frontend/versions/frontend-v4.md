# Frontend setup — v4 (aligned to current project)
...
### Updates to GERDA Dispatch View

- **New Properties:**
  - `RecommendedProjectName` and `CurrentProjectName` added to `Ticket` model.
- **Design Tokens:**
  - Gradient colors and success states now use design tokens.
- **Consistency:**
  - All hardcoded styles replaced with tokens from `design-system.css`.

### Design System Alignment

- **Dark Mode:**
  - Ensure all views support dark mode variables.
- **Typography:**
  - Fonts (`Rajdhani` and `Inter`) are consistent across views.
- **Spacing and Shadows:**
  - Tokens like `--spacing-lg` and `--shadow-md` applied consistently.